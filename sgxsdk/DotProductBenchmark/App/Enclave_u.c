...
static sgx_status_t SGX_CDECL Enclave_ocall_dot_product(void* pms)
{
	ms_ocall_dot_product_t* ms = SGX_CAST(ms_ocall_dot_product_t*, pms);
	ocall_dot_product(ms->ms_arr1, ms->ms_arr2, ms->ms_result, ms->ms_cnt);
...
static const struct {
	...
	12,
	{
		...
		(void*)Enclave_ocall_dot_product,
...
sgx_status_t ecall_dot_product(sgx_enclave_id_t eid, sgx_status_t* retval, int* arr1, int* arr2, int* result, size_t cnt)
{
	sgx_status_t status;
	ms_ecall_dot_product_t ms;
	ms.ms_arr1 = arr1;
	ms.ms_arr2 = arr2;
	ms.ms_result = result;
	ms.ms_cnt = cnt;
